...
impl Staging {
    ...
    pub fn republish(&self) {
        ...
        if let Some(e) = err {
            let _ = self
                .app
                .emit("byk://error", serde_json::json!({ "message": e }));
        }
    }

    fn emit_text(&self) {
        let text = self.text.lock().unwrap().clone();
        let _ = self.app.emit("byk://staging", serde_json::json!({ "text": text }));
    }
    ...
    pub fn set_recording(&self, recording: bool) {
        let _ = self
            .app
            .emit("byk://recording", serde_json::json!({ "recording": recording }));
    }
    ...
    pub fn error(&self, message: &str) {
        *self.last_error.lock().unwrap() = Some(message.to_string());
        let _ = self
            .app
            .emit("byk://error", serde_json::json!({ "message": message }));
    }

    /// 提交成功反馈
    pub fn committed(&self) {
        let _ = self.app.emit("byk://committed", ());
    }
}
