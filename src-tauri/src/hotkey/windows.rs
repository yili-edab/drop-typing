...
//!
//! 与 macOS 版本对照：
//! - Right Win（Key::MetaRight）→ 输入/提交通道（macOS 的右 ⌘）
//! - Right Alt（Key::AltGr）→ 修正通道（macOS 的右 ⌥）
//! - Right Shift（Key::ShiftRight）→ 指令通道（macOS 的右 ⇧）
//!
//! Windows 低级键盘钩子无需辅助功能权限（macOS 需要 AX 权限），
//! 但部分杀毒软件可能将全局键盘监听标记为可疑行为。
